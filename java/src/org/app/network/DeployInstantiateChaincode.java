		...
		try {
			...
			for (ProposalResponse res : response) {
				Logger.getLogger(DeployInstantiateChaincode.class.getName()).log(Level.INFO,
						Config.CHAINCODE_1_NAME + "- Chain code deployment " + res.getStatus() + " - " + res.getMessage());
			}
			...
			//if chain is newly created, you have to call init.
			//  if chain is already exists, comment code below.
//			String[] arguments = { "" };
...
//			for (ProposalResponse res : response) {
//				Logger.getLogger(DeployInstantiateChaincode.class.getName()).log(Level.INFO,
//						Config.CHAINCODE_1_NAME + "- Chain code instantiation " + res.getStatus() + " - " + res.getMessage());
//			}
	...
	public static void deploy_on_org1() {
		try {
			CryptoSuite cryptoSuite = CryptoSuite.Factory.getCryptoSuite();
